...
export default function NewIdeaPage() {
    ...
    async function onSubmit(data: IdeaFormValues) {
        try {
            // In demo mode, we'll use a placeholder user ID.
            const demoUserId = '5a99a46a-31b3-4798-8224-74ce3585d41c'; // From seed script

            const file = data.prototype[0];
            const fileExtension = file.name.split('.').pop();
            const fileName = `${uuidv4()}.${fileExtension}`;
            const filePath = `${demoUserId}/${fileName}`;
